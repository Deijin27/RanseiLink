...
public class NSMDL
{
    ...
    public NSMDL(List<Model> models)
    {
        Models = models;
    }
    ...
    public class Model
    {
        ...
        public Model(string name, BinaryReader br)
        {
            Name = name;
        ...
        public class PolymeshData
        {
            ...
            public PolymeshData(string name) 
            {
                Name = name;
            }

            public PolymeshData(string name, BinaryReader br)
            {
                Name = name;
                var f = br.ReadUInt16();
        ...
        private void ReadPolymeshes(BinaryReader br)
        {
            ...
            {
                var d = new PolymeshData(nodeRadixDict.Names[i], br);
        ...
        public class Material
        {
            ...
            public Material(string texture, string palette, string name)
            {
                Texture = texture;
                Palette = palette;
                Name = name;
            ...
            public Material(BinaryReader br, string texture, string palette, string name)
            {
                Texture = texture;
                Palette = palette;
                Name = name;
            ...
            public void WriteTo(BinaryWriter bw)
            {
                ...
                if (Flag.HasFlag(MATFLAG.EFFECTMTX))
                {
                    if (EffectMtx == null)
                    {
                        throw new Exception($"{nameof(NSMDL)}: has {nameof(Flag)} {nameof(MATFLAG.EFFECTMTX)}, but {nameof(EffectMtx)} is not set");
                    }
                    for (int i = 0; i < 0x10; i++)
                    {
        ...
        private void ReadMaterials(BinaryReader br)
        {
            ...
            for (int i = 0; i < materialRadixDict.Names.Count; i++)
            {
                ...
                var m = new Material(br,
                    texture: materialTextures[i],
                    palette: materialPalettes[i],
                    name: materialRadixDict.Names[i]
                    );
                Materials.Add(m);
            }
        }
        ...
        {
            public Polygon(ushort itemTag, SHPFLAG flag, string name)
            {
                ItemTag = itemTag;
                Flag = flag;
                Name = name;
                Commands = new List<PolygonDisplayCommand>();
            }
            public Polygon(ushort itemTag, SHPFLAG flag, string name, List<PolygonDisplayCommand> commands)
            {
                ItemTag = itemTag;
                Flag = flag;
                Name = name;
            ...
            public ushort ItemTag { get; }
            public SHPFLAG Flag { get; }
            public string Name { get; }
            public List<PolygonDisplayCommand> Commands { get; }
        }
        ...
        private void WritePolygons(BinaryWriter bw)
        {
            ...
            for (int i = 0; i < Polygons.Count; i++)
            {
                ...
                for (int cmdIdx = 0; cmdIdx < poly.Commands.Count; cmdIdx += 4)
                {
                    ...
                    foreach (var c in commandSet)
                    {
                        var paramLen = c.OpCode.ParamLength();
                        if (paramLen > 0)
                        {
                            if (c.Params == null || c.Params.Length != paramLen)
                            {
                                throw new Exception($"{nameof(PolygonDisplayCommand)} incorrect params length");
                            }
