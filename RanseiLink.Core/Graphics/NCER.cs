...
public class CEBK
{
    ...
    public CEBK(BinaryReader br)
    {
        ...
        for (int i = 0; i < header.NumberOfBanks; i++)
        {
            ...
            bank = bank.OrderBy(i => i.Priority).ThenBy(i => i.CellId).ToArray();
...
public struct Cell
{
    ...
    private static Size CellSize(Shape shape, Scale scale)
    {
        return shape switch
        {
            Shape.Square => scale switch
            {
                Scale.Small => new Size(8, 8),
                Scale.Medium => new Size(16, 16),
                Scale.Large => new Size(32, 32),
                Scale.XLarge => new Size(64, 64),
                _ => throw new ArgumentException($"Parameter {nameof(scale)} has invalid value {scale}"),
            },
            Shape.Wide => scale switch
            {
                Scale.Small => new Size(16, 8),
                Scale.Medium => new Size(32, 8),
                Scale.Large => new Size(32, 16),
                Scale.XLarge => new Size(64, 32),
                _ => throw new ArgumentException($"Parameter {nameof(scale)} has invalid value {scale}"),
            },
            Shape.Tall => scale switch
            {
                Scale.Small => new Size(8, 16),
                Scale.Medium => new Size(8, 32),
                Scale.Large => new Size(16, 32),
                Scale.XLarge => new Size(32, 64),
                _ => throw new ArgumentException($"Parameter {nameof(scale)} has invalid value {scale}"),
            },
            _ => throw new ArgumentException($"Parameter {nameof(shape)} has invalid value {shape}"),
        };
    }
