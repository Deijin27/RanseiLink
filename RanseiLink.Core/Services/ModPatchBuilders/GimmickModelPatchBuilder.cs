...
[PatchBuilder]
public class GimmickModelPatchBuilder(IOverrideDataProvider overrideProvider) : IPatchBuilder
{
    public void GetFilesToPatch(ConcurrentBag<FileToPatch> filesToPatch, PatchOptions patchOptions)
    {
        ...
        foreach (var file in overrideProvider.GetAllDataFilesInFolder(Path.Combine("graphics", "ikusa_obj")))
        {
            if (file.IsOverride)
            {
