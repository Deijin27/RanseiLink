...
public class ScenarioPokemonServiceTests
{
    ...
    {
        Assert.Throws<ArgumentOutOfRangeException>(() => service.Retrieve((int)ScenarioId.TheLegendOfRansei).Retrieve(-1));

        Assert.Throws<ArgumentOutOfRangeException>(() => service.Retrieve((int)ScenarioId.TheLegendOfRansei).Retrieve(200));

        Assert.Throws<ArgumentOutOfRangeException>(() => service.Retrieve(20).Retrieve(2));
    }
